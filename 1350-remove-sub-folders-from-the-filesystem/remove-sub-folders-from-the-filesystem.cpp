...
public:
    vector<string> removeSubfolders(vector<string>& folder) {
        // Sort the folders lexicographically so parent folders come before
        // their subfolders
        ...
        for (int i = 1; i < folder.size(); i++) {
            ...
            // compare(0, lastFolder.size(), lastFolder) checks if folder[i]
            // starts with lastFolder If it doesn't start with lastFolder
            // (returns != 0), then it's not a subfolder
