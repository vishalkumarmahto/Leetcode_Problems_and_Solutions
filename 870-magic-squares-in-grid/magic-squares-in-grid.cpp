...
private:
    bool isMagicSquare(vector<vector<int>>& grid, int rowStart, int colStart) {
        ...
        int sum = grid[rowStart][colStart] + grid[rowStart][colStart + 1] +
                  grid[rowStart][colStart + 2];

        // Check rows
        for (int i = rowStart; i < rowStart + 3; i++) {
            if (grid[i][colStart] + grid[i][colStart + 1] +
                    grid[i][colStart + 2] !=
                sum) {
                return false;
            }
        ...
        // Check columns
        for (int j = colStart; j < colStart + 3; j++) {
            if (grid[rowStart][j] + grid[rowStart + 1][j] +
                    grid[rowStart + 2][j] !=
                sum) {
                return false;
            }
        ...
        if (grid[rowStart][colStart + 2] + grid[rowStart + 1][colStart + 1] +
